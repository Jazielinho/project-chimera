...
def _calculate_metrics(times, batch_size, device):
    ...
    total_txts = batch_size * len(times)
    txts_per_sec = total_txts / np.sum(times)
...
def _generate_model_architecture(model, args):
    ...
    try:
        model.write_detailed_summary(
            ...
            input_size=(args.max_length,),
            batch_size=1,
        )

        summary_file = f"{args.arch_path}_summary.md"
        new_file = "architecture_text_encoder.md"
...
def _display_architecture(model, arch_path):
    ...
    summary_path = (
        os.path.join(directory, "architecture_text_encoder.md")
        if directory
        else "architecture_text_encoder.md"
    )
    ...
    print("\n📋 Model Summary:")
    print("   - Backbone: Transformer (pretrained embeddings)")
...
def format_report(
    ...
    report = "# Text Encoder Benchmark Results\n\n"
    report += f"**Date**: {date}\n\n"
    report += f"**Git SHA**: `{sha}`\n\n"
    report += "## Configuration\n\n"
    report += f"- **Device**: {device}\n"
    report += f"- **Max text length**: {max_length}\n"
    report += f"- **Batch size**: {metrics['batch_size']}\n"
    report += f"- **DataLoader workers**: {num_workers}\n"
    report += "\n## Performance Metrics\n\n"
    report += f"- **Throughput**: {metrics['txts_per_sec']:.2f} texts/sec\n"
    ...
    report += "\n## Model Details\n\n"
    report += "- **Backbone**: Transformer (pretrained embeddings)\n"
    report += "- **Normalization**: Lowercase, tokenization, padding\n"
    report += "- **Training**: Frozen embeddings, only projection head trainable\n"
    if arch_path:
        directory = os.path.dirname(arch_path)
        summary_path = (
            os.path.join(directory, "architecture_text_encoder.md")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Benchmark the text encoder")
    parser.add_argument(
        "--batch-size",
        type=int,
        default=64,
    ...
    parser.add_argument(
        ...
        type=int,
        default=32,
        help="Maximum text length (default: 32)",
    )
    parser.add_argument(
        "--num-workers",
        type=int,
    ...
    parser.add_argument(
        "--report-path",
        type=str,
        default="reports/bench_text_encoder.md",
    ...
    parser.add_argument(
        "--arch-path",
        type=str,
        default="reports/model_architecture_text",
        help="Path base for saving the architecture summary (default: reports/model_architecture_text)",
    )
    parser.add_argument(
        "--log-mlflow",
        action="store_true",
...
def build_transform(max_length: int):
    # Asumimos que el dataloader ya tokeniza y paddea los textos
    return None
...
def create_model(device: str):
    print("🏗️  Creating TextEncoder model")
    model = TextEncoder(embed_dim=256, freeze_backbone=True).to(device)
    total_params = sum(p.numel() for p in model.parameters())
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"   Total parameters: {total_params:,}")
    print(f"   Trainable parameters: {trainable_params:,}")
...
def print_metrics(metrics: dict, total_time: float):
    print("\n📊 Benchmark Results:")
    print("=" * 30)
    print(f"Throughput: {metrics['txts_per_sec']:.2f} texts/sec")
...
def log_to_mlflow(args, metrics, total_time, arch_generated):
    ...
    mlflow.set_experiment("Text Encoder Benchmark")
    with mlflow.start_run(
        run_name=f"text_encoder_benchmark_{time.strftime('%Y%m%d_%H%M%S')}"
    ):
        ...
        mlflow.log_metric("throughput_txts_per_sec", metrics["txts_per_sec"])
...
def main():
    print("🚀 Starting TextEncoder Benchmark")
    print("=" * 50)
    args = parse_args()
    transform = build_transform(args.max_length)
